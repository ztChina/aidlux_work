...
{
    if (line > 7) return;  // OLED最多8页（0~7）

    OLED_WR_CMD(0xB0 + line); // 设置页地址
    OLED_WR_CMD(0x00);        // 设置列低地址
    OLED_WR_CMD(0x10);        // 设置列高地址

    for (uint8_t n = 0; n < 128; n++)
    {
        OLED_WR_DATA(0x00);   // 每列清空为0
    }
}


void Display_Speeds(float v_left, float v_right)
{
    char buf[20];
    ...
    // 显示左轮速度，保留两位小数
    snprintf(buf, sizeof(buf), "Left: %.2f m/s", v_left);
    OLED_ShowString(0, 0, buf, 12);

    // 显示右轮速度，保留两位小数
    snprintf(buf, sizeof(buf), "Right: %.2f m/s", v_right);
