...
class WeNetDecoder:
    ...
    def get_dict(self,configs):
        ...
        """
        char_dict = {}
        with open(configs["dict"], 'r',encoding="utf-8") as fin:
            for line in fin:
    ...
    def decoder_rescoring(self,noLog=False):
        ...
        # print(f"Results is : {''.join(content)}")
        # if not noLog:
...
if __name__ == '__main__':
    weNetDecoder = WeNetDecoder('./models/model_onnx.yaml')
    for file_name in os.listdir('./python/data'):
        print(file_name)
        with open("./python/data/"+file_name,'rb') as audiostream:
            t1=time.time()
            audiostream.read(44)
            ...
            for dataflow in tqdm(iter(lambda:audiostream.read(8000),"")):
                ...
                x=weNetDecoder.ctc_prefix_beam_search_purn_try()
                # print(dataflow)
            # pdb.set_trace()
            ...
            print(f"Cost time ： {int((t2-t1)*1000)} ms")
